...
class ControllerServer():
    ...
    def mainloop(self, itr, ctrl):
        """
        itr (Interrupt Channel): itr 代表中断通道，它是一个 socket 连接，用于实时数据传输，如控制器的输入报告（按钮按下、摇杆移动、陀螺仪数据等）。这个通道主要用于发送实时的控制器状态给 Switch，并接收来自 Switch 的即时响应。
        ctrl (Control Channel): ctrl 代表控制通道，这也是一个 socket 连接，但主要用于非实时的数据传输，比如设备配对、连接管理、控制器配置等。在上面的代码中，ctrl 并没有直接用于数据的发送和接收，它主要用于建立和管理与 Switch 的连接。
        """
